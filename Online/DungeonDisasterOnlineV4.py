...
import socket
import pickle
from ObjectCodeV5 import Player, Enemy,bulletArray, Projectile
...
def create_enemies (num_enemies, player, player2):
    ...
    for i in range(num_enemies):
        ...
        if enemy_type == 1 and enemy_target == 0:
            player.online_enemy_array.append([(random.randint(wall_limit_x1, wall_limit_x2)), (random.randint(wall_limit_y1, wall_limit_y2)), "R", player])
        elif enemy_type == 2 and enemy_target == 0:
            player.online_enemy_array.append([(random.randint(wall_limit_x1, wall_limit_x2)), (random.randint(wall_limit_y1, wall_limit_y2)), "M", player])
        elif enemy_type == 1 and enemy_target == 1:
            player.online_enemy_array.append([(random.randint(wall_limit_x1, wall_limit_x2)), (random.randint(wall_limit_y1, wall_limit_y2)), "R", player2])
        elif enemy_type == 2 and enemy_target == 1:
            player.online_enemy_array.append([(random.randint(wall_limit_x1, wall_limit_x2)), (random.randint(wall_limit_y1, wall_limit_y2)), "M", player2])
...
def BottomHitbox(player, player2):
    ...
    if player.x_pos < bottom_hitbox[0] + bottom_hitbox[2] and player.x_pos + player.width > bottom_hitbox[0]:
        if player.y_pos + player.height > bottom_hitbox[1] and player.y_pos < bottom_hitbox[1] + bottom_hitbox[3]:
            ...
            #Create the enemies
            if player.ID == 0:
                create_enemies (2 + current_level, player, player2)
            #spawnEnemies(player)
...
def main_game_loop():
    ...
    while game_loop == True:
        ...
        print(player1.online_enemy_array)
        player1.movePlayer()
