...
#Here the server will be waiting for clients to connect to the server
...
gameData = [[Player(1340, 50, 50,0,1,map1, player_images, 0),Player(500, 50, 50,0,1,map1, player_images, 1)], [Player(1340, 50, 50,0,1,map2, player_images, 0),Player(500, 50, 50,0,1,map2, player_images, 1)], [Player(1340, 50, 50,0,1,map3, player_images, 0),Player(500, 50, 50,0,1,map3, player_images, 1)]]


#This procedure is responsible for receiving data and sending data back to the client
def client_thread(conn, playerID, gameID):
    
    #This will be sending the starting object code to the player
    conn.send(pickle.dumps(gameData[gameID][playerID]))
    print("Connection Made. New thread made")
    print("Player Number: ", playerID)
    print("Game Number: ", gameID)
    ...
    while True:
        try:
            ...
            #Update the game data for the particular game
            gameData[gameID][playerID] = dataReceived
            ...
            if not dataReceived:
                ...
                if playerID == 1:
                    reply = gameData[gameID][0]
                elif playerID == 0:
                    reply = gameData[gameID][1]
...
while True:
    ...
    start_new_thread(client_thread, (conn, currentPlayer, currentGame))
    #After creating a new thread, increase the player number. If it is too large, increase the game number
    currentPlayer += 1
    if currentPlayer == 2:
        currentPlayer = 0
        currentGame +=1
