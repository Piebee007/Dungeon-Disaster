...
#This section is responsible for creating the connection with the server
client_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
#Here is the IP address for the server
serverIP = "192.168.1.106"
port = 5555
address = (serverIP ,port)

#This function will be used to connect the client to the server after completing the quiz
def connect_client():
    try:
        client_socket.connect(address)
        #Connecting to the server will return the player's starting object code
        return pickle.loads(client_socket.recv(4096))
    except:
        pass

#This function will be used for sending the player's object code
def send(data_to_send):
    try:
        client_socket.send(pickle.dumps(data_to_send))
        #Sending data will always return the other player's object code
        return pickle.loads(client_socket.recv(4096))
    except socket.error as error:
        print(error)

#This procedure will create enemies for both the players' game
def create_enemies (num_enemies, player, player2):
    #Empty the player's enemy array
    player.online_enemy_array = []
    #Repeat this for loop by the number of enemies that are intended to be made
    for i in range(num_enemies):
        #Randomly select the enemies type and target
        enemy_type = random.randint(1,2)
        enemy_target = random.randint(0,1)
        #Add the data to the online enemy array which will be interpretted by the players
        if enemy_type == 1 :
...
def main_game_loop():
    ...
    while game_loop == True:
        ...
        #Receive the other player's object code after sending the current player's object code
        player2 = send(player1)
        #Create bullets whilst the other pl
        ...
        #After sending the player's object code, reset the online bullet array to stop the program from creating the same bullet   
        player1.online_bullet_array = []
        
        #Spawn enemies depending on who created them 
        if player1.ID == 0 :
        ...
        #Teleport player 2 to player 1's position if player 1 has moved to a new room
        if player1.ID != 0:
